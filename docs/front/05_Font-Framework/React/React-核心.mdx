...
- JSX 是 一种 JavaScript 的==语法扩展==
  ...
  - 它用于描述 UI 界面，并且可以和 JavaScript 融合在一起使用
...
- JSX 最终会被转化为 JavaScript
  ...
  ```jsx
  const element = <div onClick={() => {}}>Hello JSX</div>;
  ```

- 由于 `class` 是一个关键字，所以在 React 中需要用 `className` 来代替
...
- 与运算符 `&&`
  ...
  - **切勿直接将数字放在 `&&` 左侧**，因为数字 `0` 也会被渲染，除非强制转换为==布尔类型==（`!!`）
...
- `key` 需要满足的条件
  ...
  - key 值应该是稳定不变的，即不要在渲染时动态地生成 key，比如 `key={ Math.random() }` 的错误方式
...
- `setState` 可以传入一个回调函数
  ...
  - React 在更新状态时会调用这个函数并将==上一次==的状态传递过来
  - 这个回调函数的==返回值==会作为需要更新的状态
...
- `setState` 的更新默认是==异步==的

  - 设计为异步，可以显著的==提升性能==（批处理）

  - 如果同步更新了 *state* ，但还没有执行 *render* 函数，会导致 *state* 和 *props* 不能==保持同步==
    ...
    class App extends React.Component {
      handleClick() {
        ...
        this.setState({ 
          ...
          console.log(this.state.name); // 更新为 "Robin"
        });
      }
    }
    ```

- 连续调用 `setState` 更新，进行==批处理==

  - React 会将多次更新==合并成一个对象==，然后进行更新
  - 在源码中，通过 `do {} while(true)` 循环进行==遍历链表==
...
- 在 **React 18** 之前，在 `setTimeout` 和 ==原生 DOM 事件== 中会做==同步==处理
...
- 在 React 18 之后，所有的 `setState` 更新都变成==异步==处理（==自动批处理==）

  <img src="./images/image-20230811234120763.png" alt="image-20230811234120763" style="zoom:80%;" />

  - 在极少数情况下，需要强制同步应用特定的 *state* 更新，此时可以将其包装在 `flushSync` 中
...
- 作用：允许在不添加额外节点的情况下将子元素组合，可以使用==语法糖== `<>...</>` 代替
...
- 错误边界可以捕获发生在整个子组件树的==渲染期间==、==生命周期==以及==构造函数==中的错误

  - 无法捕获事件处理、异步代码、服务端渲染、它自己抛出的错误
  - 函数组件要想实现错误边界组件，需要自己实现类似的功能（HOC），或者使用 [react-error-boundary](https://www.npmjs.com/package/react-error-boundary)
...
- 内联样式是官方推荐的一种 CSS 样式的写法
  ...
  - `style` 属性接收一个采用==小驼峰命名==属性的 JavaScript 对象
  - 并且可以引用 *state* 和 *props* 中的变量来设置相关的样式

  ```jsx
  export default function App({ color }) {
    return (
      <div style={{ fontSize: 20, color }}>Hello React!</div>
    );
  }
  ```
...
- 全局样式：将 CSS 编写到一个单独的文件，之后再进行引入
  ...
  export default function App() {
    return <div className="wrapper">全局样式</div>;
  }
  ```

- 最大的缺点：不同 CSS 文件中的同名类名会导致==样式冲突==



### 模块化样式

- 相比于全局样式，**CSS Module** 解决了==局部作用域==的问题

  - 本质上是为每个组件的类名添加了==唯一== `hash`
...
export default function App() {
  return <div className={title}>全局样式</div>;
}
```
...
- 缺点：CSS 中无法读取 JavaScript 变量值，无法生成==动态==样式
...
- CSS in JS 的模式：将样式（CSS）也写入到 JavaScript 中的方式，并且可以方便的使用 JavaScript 变量
...
<div style={{ marginLeft: 32, marginTop: -10 }}>
  ...
  ```jsx
  import React from 'react';
  import { AppWrapper } from './App_style';
  
  export default function App() {
    return <AppWrapper>全局样式</AppWrapper>;
  }
  ```

  </TabItem>

  <TabItem value="App_style.js" label="App_style.js">
...
- 使用该样式组件时支持传递 *props*，动态设置样式，在样式文件中通过==模板字符串==获取 *props*
...
<div style={{ marginLeft: 32, marginTop: -10 }}>
  ...
  export default function App() {
    return <AppWrapper color="skyblue">全局样式</AppWrapper>;
  }
  ```
...
<div style={{ marginLeft: 32, marginTop: -10 }}>
  ...
  ```jsx
  import React from 'react';
  import { PrimaryButton } from './App_style';
  
  export default function App() {
    return <PrimaryButton color="orange">按钮</PrimaryButton>;
  }
  ```
...
<div style={{ marginLeft: 32, marginTop: -10 }}>
  ...
  export default function App() {
    return (
      <ThemeProvider theme={{ primary: '#f6da76' }}>
...
- 类组件的缺点
  - 复杂组件变得难以理解、维护
  - 难以理解的 `class`
  - 组件复用状态很难（比如多层 `Context` 嵌套，让组件逻辑变得复杂）

- **Hook** 是 React 16.8 的新增特性
  - 它可以让我们在不编写 class 的情况下使用 ==state== 以及其他 React 特性
  - 完全可选，100% 向后兼容
  - 基本可以代替所有类组件
- **Hook** 的约束规则
  - 只能在函数==最外层==调用，不能在循环、条件判断或子函数中调用
  - 只能在 React 的 ==函数组件==或 ==自定义 Hook== 中使用，不要在其他普通函数中调用
...
- `useState` 可以向组件添加一个==状态==变量

  - 参数：传递一个==初始化值==，可以传递初始值或者传递一个计算函数（传递的计算函数只在初始化时调用一次）
  - 返回值：`[state, setState]`，返回当前状态值和更新状态值的更新函数
  - `setState` 可以将 *state* 更新为不同的值并触发==重新渲染==
...
- React 只在==初次渲染==时读取初始状态，后续渲染时将其忽略



>set 函数分析

- `set` 函数仅更新==下一次==渲染的状态变量，因此无法在本轮更新后获取到最新值
...
- `set` 函数可以直接传递新状态，也可以传递一个根据==先前状态==来计算新状态的函数

  - 传递的更新函数必须是一个==纯函数==

  - React 会将更新函数放入==队列==中并重新渲染组件，在下一次渲染期间，React 将通过把队列中所有更新函数应用于先前的状态来计算下一个状态
  ...
  - 如果提供的新值与当前 *state* 相同，React 将 **跳过重新渲染该组件及其子组件**（对于对象类型 *state*，需要生成一个新对象）
    ...
    function App() {
      ...
      return (
        <div>
          ...
          <button onClick={changeInfo}>更新</button>
        </div>
      );
    }
    ```

- React 会==批量处理状态更新==，如果需要强制 React 更早地更新屏幕，可以使用 `ReactDOM.flushSync(cb)` （不推荐使用）
...
- `useEffect` 用于执行组件运行期间需要的==副作用==

  - 参数一（`setup`）：一个回调函数，在 React 执行完==更新 DOM== 操作之后，就会回调这个函数
    - 该回调函数可以选择性返回一个 **清理（cleanup）** 函数，用于移除事件监听、取消订阅等副作用
    - 在每次依赖项变更重新渲染后，React 将首先使用旧值运行 *cleanup* 函数，然后使用新值运行 *setup* 函数
    - 当组件从页面卸载后，*cleanup* 代码 将运行最后一次
  - 参数二：`setup` 代码中引用的所有响应式值的列表
    - 响应式值包括 *props*、*state* 以及所有直接在组件内部声明的变量和函数
    - 当响应值变化后（==浅比较==），*setup* 函数会自动执行
    - 如果不传递该参数，则组件每次重新渲染都会执行 *setup* 函数
...
- 模拟 `componentDidMount` 和 `componentDidUpdate`
  ...
  function App() {
    ...
    useEffect(() => {
      // 第一次执行时，相当于 componentDidMount
      // 后续 count 更新重新执行，相当于 componentDidUpdate
    }, [count]);
...
- `useRef` 可以引用一个不需要渲染的值
  - 参数：ref 对象的 `current` 属性的初始值，这个参数会首次渲染后被忽略
  - 返回值：返回一个 *ref* 对象，该对象在组件的整个生命周期==保持不变==，内部的 `current` 属性可以被读写

- 应用场景

  - 保存一个不需要渲染的值，保存的值可以在 `useEffect` 中读取到最新的值，不存在异步的问题
    ...
    function Stopwatch() {
      ...
      useEffect(() => {
        // 时刻可以读取到最新的值
        intervalRef.current++;
        console.log(intervalRef.current); // 1
      }, []);
    }
    ```

  - 引用一个 DOM 元素，操作原生 DOM
...
- `useMemo` 在每次重新渲染的时候能够==缓存==计算的结果

  - 参数一：要缓存计算值的函数，是一个没有任何参数的==纯函数==
  - 参数二：响应式变量列表
  - 返回值：计算函数的返回结果，仅在初次渲染和依赖更新时重新计算
...
- 应用场景
  ...
  - 跳过代价昂贵的重新计算
    ...
    ```jsx
    function App({ numberList }) {
      const evenNumberList = useMemo(() => 
  ...
  - 优化重新渲染子组件的性能
    ...
    - 默认情况下，当一个组件重新渲染时，React 会==递归==地重新渲染它的所有子组件
    - 在组件树嵌套层级非常深时，顶级组件无意义的数据变化会导致整个组件树重新渲染，导致性能变差
    - 在响应式变量未发生变化时，结合 `memo` 可以跳过子组件的重新渲染

    ```jsx
    import React, { memo, useMemo, useState } from 'react';
    
    const Header = memo(({ info }) => {
      console.log('re-render');
      return <h2>{info.name} - {info.age}</h2>;
    });
    
    const Counter = memo(({ count }) => <h2>{count}</h2>);
    ...
    export default function App() {
      ...
      const info = useMemo(() => ({
        /**
         * 每次执行 setCount 都会使组件重新渲染
         * 如果不使用 useMemo，相当于每次都创建了一个新对象
         * Header 组件每次都会重新渲染
         */
        name: 'Avril',
        age: 17
...
- `useCallback` 可以在多次渲染中==缓存函数==

  - 参数一：要缓存的函数

  - 参数二：响应式变量列表

  - 返回值：一个函数，仅在初次渲染和依赖更新时重新计算
...
- 应用场景
  ...
  - 优化重新渲染子组件的性能
    ...
    const Header = memo(({ info, changeInfo }) => {
      console.log('re-render');
      return (
        <>
          <h2>{info.name} - {info.age}</h2>
    ...
    export default function App() {
      const [count, setCount] = useState(0);
      const [info, setInfo] = useState({ name: 'Avril', age: 18 });
      
      const changeInfo = useCallback(() => {
        /**
         * 每次执行 setCount 都会使组件重新渲染
         * 如果不使用 useCallback，相当于每次都创建了一个新函数
         * Header 组件每次都会重新渲染
         */
        setInfo({ ...info, age: info.age + 1 });
      }, [info])
      ...
      return (
        <>
          <Header info={info} changeInfo={changeInfo} />
          <Counter count={count} />
          <button onClick={() => setCount(count + 1)}>+1</button>
        </>
      );
...
- `useId` 是一个用于生成==横跨服务端和客户端==的稳定的唯一 ID 的同时避免 hydration 不匹配的 hook
  - 主要用于 React 的==同构==应用开发
  - React 确保它能够与==服务端渲染==一起工作
- 不能使用 `useId` 来生成列表中的 *key*



#### useTransition

- `useTransition` 可以在==不阻塞 UI== 的情况下来更新状态

  - 返回值：`[isPending, startTransition]`
    - *isPending*：是否存在待处理的转换
    - *startTransition*：必须接收一个==同步==函数，允许将状态更新标记为延迟状态

  ```jsx
  import React, { useState, useTransition } from 'react';
  
  const list = Array(10000).fill(null).map((i, index) => index);
  
  const filterNumberList = (numberList, checked = true) => {
    if (checked) {
      return numberList.filter(n => n % 2 === 0);
    } else {
      return numberList.filter(n => n % 2 !== 0);
    }
  }
  ...
  export default function App() {
    ...
    const [numberList, setNumberList] = useState(filterNumberList(list));
    ...
    const handleToggle = (e) => {
      ...
      /**
       * 不使用 startTransition
       * 在处理列表更新时, 复选框的选中状态切换明显延迟
       */
      // const newNumberList = filterNumberList(list, checked);
    ...
    return (
      <div>
        ...
        { isPending ? 'Loading....' : numberList.map(n => <div key={n}>{n}</div>) }
      </div>
    );
  }
  ```

- React 会立即执行传递给 `startTransition` 的函数

  - 但是在它运行的同时安排的任何状态更新都被标记为==延迟更新==



#### useDeferredValue

- `useDeferredValue` 接受一个值，并返回该值的新副本

  - 该副本的更新将会被延迟更新（相对于那些需要紧急更新的内容）
  - 组件件更新时，React 将会先尝试使用旧值进行重新渲染（因此它将返回旧值）
  - 然后再在后台使用新值进行另一个重新渲染
  ...
  export default function App() {
    const [isEven, setIsEven] = useState(true);
    const [numberList, setNumberList] = useState(filterNumberList(list));
    const deferredNumberList = useDeferredValue(numberList);
    ...
    const handleToggle = (e) => {
       ...
       * 列表更新被降低优先级处理
       */
      setNumberList(filterNumberList(list, checked));
    }
    ...
    return (
      <div>
        ...
        { deferredNumberList.map(n => <div key={n}>{n}</div>) }
      </div>
    );
  }
  ```

- 由 `useDeferredValue` 引起的后台重新渲染在提交到屏幕之前不会触发 *Effect*

- 结合 `Suspense`

  - 如果由于新值引起的后台更新导致 UI 暂停，用户将不会看到 *fallback* 效果
  - 将看到旧的延迟值，直到数据加载完成
...
- React 延迟实现
  ...
  - 当 `useDeferredValue` 接收到与之前不同的值
  - 除了当前渲染，它还会安排一个后台重新渲染（可以被==中断==的）
  - 当触发的新的更新，React 会从头开始==重新启动后台渲染==
...
- `useContext` 用于读取和订阅组件中的 *Context*

  - 参数：一个 *Context* 对象，由 `React.createContext` 创建
  - 返回值：由 *Provider* 共享的 `value` 值
...
- 使用 *Context* 传递数据时，在传递对象和函数时可以借助 *useMemo* 和 *useCallback* 优化重新渲染
...
- `useReducer` 用于处理复杂的 *state*，仅仅是 *useState* 的一种替代方案

  - 参数列表
    - 参数一（*reducer*）：用于更新 *state* 的纯函数
    - 参数二：初始状态值
    - 参数三：用于计算初始值的函数
  - 返回值：`[state, dispatch]`
...
- `useImperativeHandle` 用于自定义由 `ref` 暴露出来的句柄，需要结合 `forwardRef` 使用

  - 参数一：从 *forwardRef* 渲染函数中获得的第二个参数
  - 参数二：一个包含想暴露的属性和方法的==对象==
...
- **不要滥用 ref**，**如果可以通过 props 实现，那就不应该使用 ref**



#### useLayoutEffect

- `useLayoutEffect` 和 `useEffect` 非常地相似，区别在于

  - *useEffect* 会在渲染的内容==更新到 DOM 上后==执行，不会阻塞 DOM 的更新
  - *useLayoutEffect* 会在渲染的内容==更新到 DOM 上前==执行，会阻塞 DOM 的更新
  - 可以用于在浏览器重新绘制屏幕前计算布局

  <img src="./images/image-20211107234705550.png" alt="image-20211107234705550" style="zoom:80%;" />
...
- 应用实例
  ...
  - 获取元素的滚动位置
    ...
    /**
     * 获取滚动位置
     * @param {HTMLElement} el 要监听滚动的元素 
     */
    const useScrollPosition = (el) => {
...
- 需要指定 `in` 属性来触发==进入==或==退出==状态
  ...
  <img src="./images/CSSTransition.gif" alt="CSSTransition" style="zoom:80%;" />
...
<img src="./images/SwitchTransition.gif" alt="SwitchTransition" style="zoom:80%;" />
...
<img src="./images/TransitionGroup.gif" alt="TransitionGroup" style="zoom:80%;" />
